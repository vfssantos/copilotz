...
const jsonSchema = async ({ schema, module }) => {
    const shortedSchema = jsonSchemaToShortSchema(schema, { detailed: false });
    const functionSpec = jsonSchemaToFunctionSpec(schema);

    const fn = async (data) => {
        const result = await module({ data, shortSchema: shortedSchema })
        return result;
    }
