...
const taskManager = async ({ answer, threadLogs, instructions, input, audio, user, thread, options, iterations = 0 }, res) => {
    ...
    try {
        ...
        // Define the output schema for taskManager
        taskManagerAgentResponse = validate(
            jsonSchemaToShortSchema(functionCallAgentResponse),
    ...
    taskManagerAgentResponse.functions?.forEach(async (func) => {
    ...
    if (Object.keys(updateTaskPayload).length) {
        ...
        if (updateTaskPayload.currentStep !== currentStep._id && iterations < maxIter) {
            ...
            return await taskManager({
                ...
                thread,
                threadLogs: [
                    ...taskManagerAgentResponse?.prompt?.slice(1),
                    {
                        role: 'assistant',
                        content: JSON.stringify(taskManagerAgentResponse)
                    }
                ],
                options,
